...
<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>Overview: See your dispatches or team status. Route: Start your patrol route. Use the sidebar toggles on mobile.</p> <p>Example: Use 10-codes like "10-42" to mark yourself off duty.</p> </div> <div class="dashboard-container"> <div class="sidebar" id="unitsSidebar"> <button class="toggle-sidebar tooltip" onclick="toggleSidebar('unitsSidebar')"> Toggle Units <span class="tooltip-text">Show/hide the Active Units sidebar</span> </button> <h3 class="text-lg font-semibold mb-2">Active Units</h3> <select id="unitFilter" class="bg-gray-700 text-white p-2 rounded w-full mb-2 tooltip" onchange="updateUnitsList()"> <option value="">All Units</option> <option value="Online">Online</option> <option value="Offline">Offline</option> <span class="tooltip-text">Filter units by online/offline status</span> </select> <div id="unitsList"></div> </div> <div class="main-content"> <div class="flex space-x-4 mb-4"> <button onclick="showTab('overview')" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow tooltip"> Overview <span class="tooltip-text">View team status and active dispatches</span> </button> <button onclick="showTab('route')" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow tooltip"> Route <span class="tooltip-text">Manage your assigned patrol route</span> </button> </div> <div id="dashboardContent"></div> <div class="nato-cheat-sheet"> <h4 class="text-lg font-semibold mb-2">NATO Phonetic Cheat Sheet</h4> <p>A - Alpha | B - Bravo | C - Charlie | D - Delta | E - Echo | F - Foxtrot | G - Golf | H - Hotel | I - India | J - Juliet | K - Kilo | L - Lima | M - Mike</p> <p>N - November | O - Oscar | P - Papa | Q - Quebec | R - Romeo | S - Sierra | T - Tango | U - Uniform | V - Victor | W - Whiskey | X - Xray | Y - Yankee | Z - Zulu</p> </div> </div> <div class="sidebar" id="dispatchSidebar"> <button class="toggle-sidebar tooltip" onclick="toggleSidebar('dispatchSidebar')"> Toggle Dispatches <span class="tooltip-text">Show/hide the Active Dispatches sidebar</span> </button> <h3 class="text-lg font-semibold mb-2">Active Dispatches</h3> <div id="dispatchList"></div> </div> </div> <div id="alert" class="hidden"></div> <script> let isLoaded = false; console.log('Index: Checking if data is loaded, isLoaded:', isLoaded); if (!isLoaded) { console.log('Index: Calling loadData for Dashboard'); loadData(() => { isLoaded = true; console.log('Index: loadData callback executed, isLoaded:', isLoaded); console.log('Index: employeesData after load:', employeesData); console.log('Index: dispatchData after load:', dispatchData); showTab('overview'); updateUnitsList(); updateDispatchList(); }); } else { console.log('Index: Data already loaded, skipping loadData'); }
...
;             break;         case 'properties.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>View all properties. Click underlined names (e.g., "Axel Apartments") for details with map. Suspended properties are managed in Manager page.</p> </div> <h3 class="text-xl font-semibold mb-2">Properties</h3> <div id="propertiesList" class="bg-gray-800 p-4 rounded-lg overflow-x-auto shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('Properties: Calling loadData for Properties'); loadData(() => { console.log('Properties: loadData callback executed'); console.log('Properties: propertiesData after load:', propertiesData); const customProperties = JSON.parse(localStorage.getItem('customProperties') || '[]'); customProperties.forEach(prop => { if (!propertiesData.some(p => p.id === prop.id)) propertiesData.push(prop); });
...
;             break;         case 'people.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>Search by Name (e.g., "John Doe"), ID (e.g., "P001"), or use filters (e.g., CTN Status: "CTN Issued", Property: "PROP001"). List is empty until you search.</p> </div> <h3 class="text-xl font-semibold mb-2">People Search</h3> <div class="bg-gray-800 p-4 rounded-lg mb-4 shadow"> <p class="text-sm text-gray-400 mb-2">Search by Name, ID, or filters below. Full list not shown by default (demo only). Enter criteria and click Search.</p> <input type="text" id="simpleSearch" placeholder="Name or ID" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/3 mb-2"> <div class="flex flex-wrap gap-2"> <select id="ctnFilter" class="bg-gray-700 text-white p-2 rounded"><option value="">CTN Status</option><option value="N/A">N/A</option><option value="CTN Issued">CTN Issued</option><option value="VCTW">VCTW</option></select> <select id="behaviorFilter" class="bg-gray-700 text-white p-2 rounded"><option value="">Behavior</option><option value="Friendly">Friendly</option><option value="Cautious">Cautious</option><option value="Hostile">Hostile</option><option value="Unknown">Unknown</option></select> <input type="text" id="propertyFilter" placeholder="Property ID" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/4"> <button onclick="searchPeople()" class="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded shadow">Search</button> </div> </div> <div id="peopleResults" class="bg-gray-800 p-4 rounded-lg overflow-x-auto shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('People: Calling loadData for People'); loadData(() => { console.log('People: loadData callback executed'); console.log('People: peopleData after load:', peopleData); });
...
;             break;         case 'dispatch.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>Active: View and manage current calls (assign officers, mark clear). History: See completed calls (restore if needed). Officers see only their assigned calls.</p> </div> <div class="flex space-x-4 mb-4"> <button onclick="showTab('active')" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow">Active</button> <button onclick="showTab('history')" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow">History</button> </div> <div id="dispatchContent" class="bg-gray-800 p-4 rounded-lg shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('Dispatch: Calling loadData for Dispatch'); loadData(() => { console.log('Dispatch: loadData callback executed'); console.log('Dispatch: dispatchData after load:', dispatchData); showTab('active'); });
function showTab(tab) {
...
;             break;         case 'reports.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>Submit a report below (e.g., Patrol Hit, Incident). View all reports in the table. Filter by person via URL (e.g., ?personId=P001). Click Case Number to view details.</p> </div> <h3 class="text-xl font-semibold mb-2">Submit Report</h3> <div class="bg-gray-800 p-4 rounded-lg mb-4 shadow"> <div class="flex flex-wrap gap-2"> <input type="text" id="personId" placeholder="Person ID (e.g., P001)" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/4"> <input type="text" id="property" placeholder="Property ID (e.g., PROP001)" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/4"> <select id="type" class="bg-gray-700 text-white p-2 rounded"><option value="Patrol Hit">Patrol Hit</option><option value="Incident">Incident</option><option value="CTN Update">CTN Update</option></select> <input type="text" id="narrative" placeholder="Narrative" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/3"> <button onclick="submitReport()" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow">Submit</button> </div> </div> <h3 class="text-xl font-semibold mb-2">Reports</h3> <div id="reportsList" class="bg-gray-800 p-4 rounded-lg overflow-x-auto shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('Reports: Calling loadData for Reports'); loadData(() => { console.log('Reports: loadData callback executed'); console.log('Reports: reportsData after load:', reportsData); const urlParams = new URLSearchParams(window.location.search); const personId = urlParams.get('personId'); showReports(personId); });
...
;             break;         case 'officers.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>View all officers and their current status. Update your status (Officers only) or any officer's status (Managers/Supervisors/Dispatchers) using the dropdown.</p> </div> <h3 class="text-xl font-semibold mb-2">Officers Status</h3> <div id="officersList" class="bg-gray-800 p-4 rounded-lg overflow-x-auto shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('Officers: Calling loadData for Officers'); loadData(() => { console.log('Officers: loadData callback executed'); console.log('Officers: employeesData after load:', employeesData); showOfficers(); });
...
;             break;         case 'manager.html':             content = 

<div class="tips"> <h4 class="text-lg font-semibold">Tips</h4> <p>Add properties, manage employees (edit group/delete), and suspend/activate properties. All actions require confirmation.</p> </div> <h3 class="text-xl font-semibold mb-2">Add Property</h3> <div class="bg-gray-800 p-4 rounded-lg mb-4 shadow"> <div class="flex flex-wrap gap-2"> <input type="text" id="propId" placeholder="Property ID (e.g., PROP004)" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/4"> <input type="text" id="propName" placeholder="Property Name" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/4"> <input type="text" id="propAddress" placeholder="Address" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/3"> <input type="text" id="propApt" placeholder="Apt" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/6"> <input type="number" id="propHits" placeholder="Min Hits" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/6"> <input type="text" id="propNotes" placeholder="Notes" class="bg-gray-700 text-white p-2 rounded w-full sm:w-1/3"> <button onclick="addProperty()" class="bg-blue-600 hover:bg-blue-700 p-2 rounded shadow">Add</button> </div> </div> <h3 class="text-xl font-semibold mb-2">Manage Employees</h3> <div id="employeeList" class="bg-gray-800 p-4 rounded-lg mb-4 shadow overflow-x-auto"></div> <h3 class="text-xl font-semibold mb-2">Manage Properties</h3> <div id="propertyList" class="bg-gray-800 p-4 rounded-lg overflow-x-auto shadow"></div> <div id="alert" class="hidden"></div> <script> console.log('Manager: Calling loadData for Manager'); loadData(() => { console.log('Manager: loadData callback executed'); console.log('Manager: employeesData after load:', employeesData); console.log('Manager: propertiesData after load:', propertiesData); showEmployees(); showProperties(); });
